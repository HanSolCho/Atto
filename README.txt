...
- 전체 조회 시, 100개의 호스트가 모두 Unreachable 상태여도 1초 이내에 응답해야 함
 ...
 -> 해당 항목 관련하여 '호스트 상태 모니터링 API' 호출 시 host 모두가 현재 상황이 Unreachable 상태임을 확인하고 그에 맞춰 갱신해야 한다고 이해되는데 그 부분에 있어서는 해결하지 못했습니다.

 -> 만약 Unreachable 상태 확인 및 업데이트가 '호스트 현재 상태 조회 API' 에서 적용 후 '호스트 상태 모니터링 API'에서는 상태 값만 조회하는 경우라면 문제가 없습니다.

 * 개인 사정상 주말 간 개발이 불가하여 추가적인 수정은 못하게 되었습니다.
 * 시간이 가능했다면 쓰레드를 통한 병력로 100개의 'InetAddress.isReachable()' 요청을 구현하면 될것이라고 생각됩니다.
